...
from megatron.core.utils import get_gtp_weight_remat_group, log_single_rank
...
def configure_gtp_remat_from_recipe(
    *, fp4=False, fp8_recipe=None, fp8=False, calculate_per_token_loss=False,
    ...
    if gtp_nccl_ub:
        group = get_gtp_weight_remat_group()
        _initialize_gtp_symmetric_group(group)
        _initialize_gtp_symmetric_group(_ps.get_gtp_rs_shadow_group(group))
    if egtp_nccl_ub:
        group = get_gtp_weight_remat_group(is_expert=True)
        _initialize_gtp_symmetric_group(group)
        _initialize_gtp_symmetric_group(_ps.get_gtp_rs_shadow_group(group))
...
def _gtp_attach_attrs(gtp_shard, gtp_remat_group, *, is_grouped=False, expert_idx=0):
    ...
    _is_expert_group = gtp_remat_group is get_gtp_weight_remat_group(is_expert=True)
